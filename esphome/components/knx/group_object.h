...
class GroupObject {
 ...
 public:
  ...
  /**
   * returns the size of the group object in Byte. For Group objects with size smaller than 1 byte (for example Dpt 1)
   * this method will return 1.
   */
  size_t valueSize();
  /**
   * returns the size of the group object in Byte as it is in a telegram. For Group objects with size smaller than 1
   * byte (for example Dpt 1) this method will return 0.
   */
  size_t sizeInTelegram();
  /**
   * returns the pointer to the value of the group object. This can be used if a datapoint type is not supported or if
   * you want do your own conversion.
   */
  uint8_t *valueRef();
  /**
   * returns the Application Service Access Point of the group object. In reality this is just the number of the group
   * object. (in german "KO-Nr")
   */
  uint16_t asap();

#ifndef SMALL_GROUPOBJECT
  /**
   * register a callback for this group object. The registered callback will be called if the group object was changed
   * from the bus.
   */
   ...
   * @param type the datapoint type used for the conversion. If this doesn't fit to the group object the returned value
   * is invalid.
   */
...
#ifndef SMALL_GROUPOBJECT
  /**
   * return the current value of the group object. The datapoint type must be set with dataPointType(). Otherwise the
   ...
   * @param value the value the group object is set to
   *
   * The parameters must fit the group object and dhe datapoint type must be set with dataPointType(). Otherwise it will
   * stay unchanged.
   */

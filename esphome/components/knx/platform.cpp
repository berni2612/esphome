...
NvMemoryType Platform::NonVolatileMemoryType() { return _memoryType; }

void Platform::NonVolatileMemoryType(NvMemoryType type) { _memoryType = type; }

size_t Platform::readBytesUart(uint8_t *buffer, size_t length) { return 0; }

int Platform::readUart() { return -1; }

size_t Platform::writeUart(const uint8_t *buffer, size_t size) { return 0; }

size_t Platform::writeUart(const uint8_t data) { return 0; }
...
uint32_t Platform::uniqueSerialNumber() { return 0x01020304; }

size_t Platform::flashEraseBlockSize() { return 0; }
...
uint8_t *Platform::userFlashStart() { return nullptr; }

size_t Platform::userFlashSizeEraseBlocks() { return 0; }

void Platform::flashErase(uint16_t eraseBlockNum) {}

void Platform::flashWritePage(uint16_t pageNumber, uint8_t *data) {}

uint8_t *Platform::getEepromBuffer(uint16_t size) { return nullptr; }
...
void Platform::bufferEraseBlock(int32_t eraseBlockNumber) {
  ...
  memcpy(_eraseblockBuffer, userFlashStart() + eraseBlockNumber * flashEraseBlockSize() * flashPageSize(),
         flashEraseBlockSize() * flashPageSize());

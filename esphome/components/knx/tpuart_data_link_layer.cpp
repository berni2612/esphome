...
// tx states
enum { TX_IDLE, TX_FRAME, TX_WAIT_CONN };

// rx states
enum { RX_WAIT_START, RX_L_ADDR, RX_L_DATA, RX_WAIT_EOP };
      ...
      switch (_rxState) {
        ...
        case RX_L_ADDR:
          ...
          if (_RxByteCnt == 7) {
            ...
            _isEcho = (_sendBuffer != nullptr &&
                       (!((buffer[0] ^ _sendBuffer[0]) & ~0x20) && !memcmp(buffer + _convert + 1, _sendBuffer + 1, 5)));
...
TpUartDataLinkLayer::TpUartDataLinkLayer(DeviceObject &devObj, NetworkLayerEntity &netLayerEntity, Platform &platform,
                                         ITpUartCallBacks &cb)
    : DataLinkLayer(devObj, netLayerEntity, platform), _cb(cb) {}
...
bool TpUartDataLinkLayer::enabled() const { return _enabled; }

...
int KNX_Decode_Value(uint8_t *payload, size_t payload_length, const Dpt &datatype, KNXValue &value) {
  if (payload_length > 0) {
    // DPT 1.* - Binary
    if (datatype.mainGroup == 1 && datatype.subGroup >= 1 && datatype.subGroup <= 23 && datatype.subGroup != 20 &&
        !datatype.index) {
    ...
    if (datatype.mainGroup == 5 &&
        ((datatype.subGroup >= 1 && datatype.subGroup <= 6 && datatype.subGroup != 2) || datatype.subGroup == 10) &&
        !datatype.index)
    ...
    if (datatype.mainGroup == 7 &&
        (datatype.subGroup == 1 || (datatype.subGroup >= 10 && datatype.subGroup <= 13) ||
    ...
    if (datatype.mainGroup == 8 && (datatype.subGroup == 1 || datatype.subGroup == 10 || datatype.subGroup == 11) &&
        !datatype.index)
    ...
    if (datatype.mainGroup == 9 &&
        ((datatype.subGroup >= 1 && datatype.subGroup <= 11) || (datatype.subGroup >= 20 && datatype.subGroup <= 29)) &&
        !datatype.index)
    ...
    if (datatype.mainGroup == 13 &&
        (datatype.subGroup == 1 || datatype.subGroup == 2 || (datatype.subGroup >= 10 && datatype.subGroup <= 15)) &&
        !datatype.index)
...
int KNX_Encode_Value(const KNXValue &value, uint8_t *payload, size_t payload_length, const Dpt &datatype) {
  ...
  // DPT 7.001/7.010/7.011/7.012/7.013/7.600 - Unsigned 16 Bit Integer
  if (datatype.mainGroup == 7 &&
      (datatype.subGroup == 1 || (datatype.subGroup >= 10 && datatype.subGroup <= 13) || datatype.subGroup == 600) &&
      !datatype.index)
...
int valueToBusValueString(const KNXValue &value, uint8_t *payload, size_t payload_length, const Dpt &datatype) {
  ...
  for (int n = 0; n < 14; n++) {
    if (val)
      val = strValue[n];  // string terminator 0x00 will stop further assignments and init the remainig payload with
                          // zero
...
uint8_t unsigned8FromPayload(const uint8_t *payload, int index) { return (uint8_t) payload[index]; }
int8_t signed8FromPayload(const uint8_t *payload, int index) { return (int8_t) payload[index]; }
...
uint32_t unsigned32FromPayload(const uint8_t *payload, int index) {
  return ((((uint32_t) payload[index]) << 24) & 0xFF000000) | ((((uint32_t) payload[index + 1]) << 16) & 0x00FF0000) |
         ((((uint32_t) payload[index + 2]) << 8) & 0x0000FF00) | (((uint32_t) payload[index + 3]) & 0x000000FF);
}

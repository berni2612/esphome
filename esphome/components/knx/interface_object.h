...
class InterfaceObject : public SaveRestore {
 public:
  ...
  /**
   * Read the Description of a property of the interface object. The output parameters are only valid if nuberOfElements
   * is not zero.
   *
   ...
   *        only used for input if propertyId is not set. Otherwise the index of the property specified by propertyId is
   * written to it.
   *
   ...
   *
   * @param[out] numberOfElements the number of elements of the property. Zero if the interface object does not have the
   * requested property.
   *
   * @param[out] access the ::AccessLevel necessary to read/write the property.
   */
  void readPropertyDescription(uint8_t &propertyId, uint8_t &propertyIndex, bool &writeEnable, uint8_t &type,
  ...
  template<typename T> T propertyValue(PropertyID id) {
  ...
  template<typename T> void propertyValue(PropertyID id, T value) {

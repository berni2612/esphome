...
GroupObjectTableObject &BauSystemBDevice::groupObjectTable() { return _groupObjTable; }
...
void BauSystemBDevice::sendNextGroupTelegram() {
  ...
  for (uint16_t asap = startIdx; asap <= objCount; asap++) {
    ...
    goSecurity.toolAccess =
        false;  // Secured group communication never uses the toolkey. ETS knows all keys, also the group keys.
...
bool BauSystemBDevice::configured() {
  ...
  _configured = _groupObjTable.loadState() == LS_LOADED && _addrTable.loadState() == LS_LOADED &&
                _assocTable.loadState() == LS_LOADED && _appProgram.loadState() == LS_LOADED;
...
void BauSystemBDevice::groupValueWriteLocalConfirm(AckType ack, uint16_t asap, Priority priority, HopCountType hopType,
...
void BauSystemBDevice::groupValueReadLocalConfirm(AckType ack, uint16_t asap, Priority priority, HopCountType hopType,
...
void BauSystemBDevice::groupValueReadIndication(uint16_t asap, Priority priority, HopCountType hopType,
...
void BauSystemBDevice::groupValueReadAppLayerConfirm(uint16_t asap, Priority priority, HopCountType hopType,
...
void BauSystemBDevice::groupValueWriteIndication(uint16_t asap, Priority priority, HopCountType hopType,
                                                 const SecurityControl &secCtrl, uint8_t *data, uint8_t dataLength) {
#ifdef USE_DATASECURE
  DataSecurity requiredGoSecurity;

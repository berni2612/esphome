...
MessageCode CemiFrame::messageCode() const { return (MessageCode) _data[0]; }

void CemiFrame::messageCode(MessageCode msgCode) { _data[0] = msgCode; }

uint16_t CemiFrame::totalLenght() const {
  uint16_t tmp = _npdu.length() + NPDU_LPDU_DIFF;
  return tmp;
}
...
uint16_t CemiFrame::telegramLengthtRF() const { return totalLenght() - 3; }
...
void CemiFrame::fillTelegramRF(uint8_t *data) {
  ...
  data[0] = _ctrl1[1] &
            0x0F;  // KNX CTRL field for RF (bits 3..0 EFF only), bits 7..4 are set to 0 for asynchronous RF frames
  memcpy(data + 1, _ctrl1 + 2, 4);  // SA, DA
  data[5] =
      (_ctrl1[1] & 0xF0) | ((_rfLfn & 0x7) << 1) | ((_ctrl1[0] & 0x10) >> 4);  // L/NPCI field: AT, Hopcount, LFN, AET
  memcpy(data + 6, _ctrl1 + 7, len - 6);                                       // APDU

  // printHex("cEMI_fill: ", &data[0], len);
}
#endif
uint8_t *CemiFrame::data() { return _data; }

uint16_t CemiFrame::dataLength() { return _length; }
...
FrameFormat CemiFrame::frameType() const { return (FrameFormat) (_ctrl1[0] & StandardFrame); }
...
Repetition CemiFrame::repetition() const { return (Repetition) (_ctrl1[0] & RepetitionAllowed); }
...
SystemBroadcast CemiFrame::systemBroadcast() const { return (SystemBroadcast) (_ctrl1[0] & Broadcast); }
...
Priority CemiFrame::priority() const { return (Priority) (_ctrl1[0] & LowPriority); }
...
AckType CemiFrame::ack() const { return (AckType) (_ctrl1[0] & AckRequested); }
...
Confirm CemiFrame::confirm() const { return (Confirm) (_ctrl1[0] & ConfirmError); }
...
AddressType CemiFrame::addressType() const { return (AddressType) (_ctrl1[1] & GroupAddress); }
...
uint8_t CemiFrame::hopCount() const { return ((_ctrl1[1] >> 4) & 0x7); }
...
void CemiFrame::sourceAddress(uint16_t value) { pushWord(value, _ctrl1 + 2); }
...
void CemiFrame::destinationAddress(uint16_t value) { pushWord(value, _ctrl1 + 4); }
#ifdef USE_RF
uint8_t *CemiFrame::rfSerialOrDoA() const { return _rfSerialOrDoA; }

void CemiFrame::rfSerialOrDoA(const uint8_t *rfSerialOrDoA) { _rfSerialOrDoA = (uint8_t *) rfSerialOrDoA; }

uint8_t CemiFrame::rfInfo() const { return _rfInfo; }

void CemiFrame::rfInfo(uint8_t rfInfo) { _rfInfo = rfInfo; }

uint8_t CemiFrame::rfLfn() const { return _rfLfn; }

void CemiFrame::rfLfn(uint8_t rfLfn) { _rfLfn = rfLfn; }
#endif
NPDU &CemiFrame::npdu() { return _npdu; }

TPDU &CemiFrame::tpdu() { return _tpdu; }

APDU &CemiFrame::apdu() { return _apdu; }

...
class MemoryBlock {
 public:
  MemoryBlock(){};
  MemoryBlock(uint8_t *address, size_t size) : address(address), size(size) {}
...
class Memory {
 ...
 private:
  ...
  uint16_t _metadataSize =
      6 + LEN_HARDWARE_TYPE;  // accounting for 3x pushWord and pushByteArray of length LEN_HARDWARE_TYPE
};

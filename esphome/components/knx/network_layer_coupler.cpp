...
NetworkLayerCoupler::NetworkLayerCoupler(DeviceObject &deviceObj, TransportLayer &layer)
    : NetworkLayer(deviceObj, layer), _netLayerEntities{{*this, kPrimaryIfIndex}, {*this, kSecondaryIfIndex}} {
  _currentAddress = deviceObj.individualAddress();
  evaluateCouplerType();
}

NetworkLayerEntity &NetworkLayerCoupler::getPrimaryInterface() { return _netLayerEntities[0]; }

NetworkLayerEntity &NetworkLayerCoupler::getSecondaryInterface() { return _netLayerEntities[1]; }
...
void NetworkLayerCoupler::rtObjPrimary(RouterObject &rtObjPrimary) { _rtObjPrimary = &rtObjPrimary; }

void NetworkLayerCoupler::rtObjSecondary(RouterObject &rtObjSecondary) { _rtObjSecondary = &rtObjSecondary; }
...
bool NetworkLayerCoupler::isRoutedIndividualAddress(uint16_t individualAddress) {
  ...
  // 3 = all frames on point-to-point connectionless – or connection-oriented communication mode shall be negatively
  // acknowledge (NACK).
  //     This shall serve for protection purposes. (It is useful to prevent all parameterisation in one Subnetwork; the
  //     Coupler shall be protected too. A typical use case is the protection of a Subnetwork that is located outside a
  //     building)
...
void NetworkLayerCoupler::sendMsgHopCount(AckType ack, AddressType addrType, uint16_t destination, NPDU &npdu,
                                          Priority priority, SystemBroadcast broadcastType,
                                          uint8_t sourceInterfaceIndex, uint16_t source) {
  // If we have a frame from open medium on secondary side (e.g. RF) to primary side, then shall use the hop count of
  // the primary router object
  if ((_rtObjPrimary != nullptr) && (_rtObjSecondary != nullptr) && (sourceInterfaceIndex == kSecondaryIfIndex)) {
    ...
    if (mediumType ==
        DptMedium::KNX_RF)  // Probably also KNX_PL110, but this is not specified, PL110 is also an open medium
    {
      uint16_t hopCount = 0;
...
// TODO: for later: improve by putting routing algorithms in its own class/functions and only instantiate required
// algorithm (line vs. coupler)
// TODO: we could also do the sanity checks here, i.e. check if sourceAddress is really coming in from correct srcIfIdx,
// etc. (see PID_COUPL_SERV_CONTROL: EN_SNA_INCONSISTENCY_CHECK)
void NetworkLayerCoupler::routeDataIndividual(AckType ack, uint16_t destination, NPDU &npdu, Priority priority,
                                              uint16_t source, uint8_t srcIfIndex) {
  ...
  if (_couplerType == LineCoupler) {
    ...
    if (srcIfIndex == kPrimaryIfIndex) {
      ...
      if (D == 0) {
        ...
        sendMsgHopCount(ack, AddressType::IndividualAddress, destination, npdu, priority, Broadcast, srcIfIndex,
                        source);
      }
      return;
    }
    ...
    if (srcIfIndex == kLocalIfIndex) {
      ...
      _netLayerEntities[destIfidx].sendDataRequest(npdu, ack, destination, source, priority,
                                                   AddressType::IndividualAddress, Broadcast);
      return;
    }
  }
...
void NetworkLayerCoupler::dataIndication(AckType ack, AddressType addrType, uint16_t destination, FrameFormat format,
                                         NPDU &npdu, Priority priority, uint16_t source, uint8_t srcIfIdx) {
  ...
  // what made the Security Proxy(AN192) useless; now, hc 7 Telegrams are filtered as any other and the value is
  // decremented.
...
void NetworkLayerCoupler::dataConfirm(AckType ack, AddressType addrType, uint16_t destination, FrameFormat format,
                                      Priority priority, uint16_t source, NPDU &npdu, bool status, uint8_t srcIfIdx) {
  HopCountType hopType = npdu.hopCount() == 7 ? UnlimitedRouting : NetworkLayerParameter;
...
void NetworkLayerCoupler::broadcastIndication(AckType ack, FrameFormat format, NPDU &npdu, Priority priority,
                                              uint16_t source, uint8_t srcIfIdx) {
...
void NetworkLayerCoupler::broadcastConfirm(AckType ack, FrameFormat format, Priority priority, uint16_t source,
                                           NPDU &npdu, bool status, uint8_t srcIfIdx) {
  HopCountType hopType = npdu.hopCount() == 7 ? UnlimitedRouting : NetworkLayerParameter;
...
void NetworkLayerCoupler::systemBroadcastIndication(AckType ack, FrameFormat format, NPDU &npdu, Priority priority,
                                                    uint16_t source, uint8_t srcIfIdx) {
  // Send it to our local stack first
  {
    HopCountType hopType = npdu.hopCount() == 7 ? UnlimitedRouting : NetworkLayerParameter;
...
void NetworkLayerCoupler::systemBroadcastConfirm(AckType ack, FrameFormat format, Priority priority, uint16_t source,
                                                 NPDU &npdu, bool status, uint8_t srcIfIdx) {
  // Check if received frame is an echo from our sent frame, we are a normal device in this case
  if (source == _deviceObj.individualAddress()) {
    HopCountType hopType = npdu.hopCount() == 7 ? UnlimitedRouting : NetworkLayerParameter;
...
void NetworkLayerCoupler::dataIndividualRequest(AckType ack, uint16_t destination, HopCountType hopType,
                                                Priority priority, TPDU &tpdu) {
...
void NetworkLayerCoupler::dataGroupRequest(AckType ack, uint16_t destination, HopCountType hopType, Priority priority,
                                           TPDU &tpdu) {
  NPDU &npdu = tpdu.frame().npdu();
  ...
  if (isGroupAddressInFilterTable(destination)) {
    _netLayerEntities[kPrimaryIfIndex].sendDataRequest(npdu, ack, destination, _deviceObj.individualAddress(), priority,
                                                       GroupAddress, Broadcast);
  }
  // We send it to our sub line in any case
  _netLayerEntities[kSecondaryIfIndex].sendDataRequest(npdu, ack, destination, _deviceObj.individualAddress(), priority,
                                                       GroupAddress, Broadcast);
}
...
void NetworkLayerCoupler::dataBroadcastRequest(AckType ack, HopCountType hopType, Priority priority, TPDU &tpdu) {
  ...
  _netLayerEntities[kPrimaryIfIndex].sendDataRequest(npdu, ack, 0, _deviceObj.individualAddress(), priority,
                                                     GroupAddress, Broadcast);
  _netLayerEntities[kSecondaryIfIndex].sendDataRequest(npdu, ack, 0, _deviceObj.individualAddress(), priority,
                                                       GroupAddress, Broadcast);
}

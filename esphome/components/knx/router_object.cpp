...
RouterObject::RouterObject(Memory &memory) : TableObject(memory) {}
...
void RouterObject::initialize(CouplerModel model, uint8_t objIndex, DptMedium mediumType, RouterObjectType rtType,
  ...
  Property *fixedProperties[] = {
      ...
      new DataProperty(PID_MEDIUM_STATUS, false, PDT_GENERIC_01, 1, ReadLv3 | WriteLv0,
                       (uint16_t) 0),  // 0 means communication is possible, could be set by datalink layer or bau to 1
                                       // (comm impossible)
  ...
  Property *model1xProperties[] = {
      ...
      new DataProperty(PID_MAIN_LCCONFIG, true, PDT_BITSET8, 1, ReadLv3 | WriteLv0,
                       (uint8_t) 0),  // Primary: data individual (connless and connorient) + broadcast
      new DataProperty(PID_SUB_LCCONFIG, true, PDT_BITSET8, 1, ReadLv3 | WriteLv0,
                       (uint8_t) 0),  // Secondary: data individual (connless and connorient) + broadcast
      new DataProperty(PID_MAIN_LCGRPCONFIG, true, PDT_BITSET8, 1, ReadLv3 | WriteLv0,
                       (uint8_t) 0),  // Primary: data group
      new DataProperty(PID_SUB_LCGRPCONFIG, true, PDT_BITSET8, 1, ReadLv3 | WriteLv0,
                       (uint8_t) 0),  // Secondary: data group
  };
  ...
  Property *model20Properties[] = {
      new DataProperty(PID_OBJECT_INDEX, false, PDT_UNSIGNED_CHAR, 1, ReadLv3 | WriteLv0,
                       objIndex),  // Must be set by concrete BAUxxxx!
  ...
  Property *tableProperties[] = {
      new DataProperty(PID_COUPLER_SERVICES_CONTROL, true, PDT_GENERIC_01, 1, ReadLv3 | WriteLv0,
                       (uint8_t) 0),  // written by ETS TODO: implement
      new DataProperty(PID_FILTER_TABLE_USE, true, PDT_BINARY_INFORMATION, 1, ReadLv3 | WriteLv0,
                       (uint16_t) 0),  // default: invalid filter table, do not use, written by ETS
      new FunctionProperty<RouterObject>(
  ...
  allPropertiesCount += ((mediumType == DptMedium::KNX_RF) || (mediumType == DptMedium::KNX_IP))
                            ? 1
                            : 0;  // PID_RF_ENABLE_SBC and PID_IP_ENABLE_SBC
  ...
  if (useHopCount) {
    // TODO: Primary side: 5 for line coupler, 4 for backbone coupler, only exists if secondary is open medium without
    // hop count Do we need to set a default value here or is it written by ETS?
  ...
  if (mediumType == DptMedium::KNX_RF) {
    allProperties[i++] = new FunctionProperty<RouterObject>(
  ...
  if (isCommand) {
    ...
    switch (srvId) {
      ...
      case ClearGroupAddress: {
        ...
        popWord(startAddress, &data[2]);
        popWord(endAddress, &data[4]);
        resultData[0] = statusClearSetGroupAddress(startAddress, endAddress, false) ? ReturnCodes::Success
                                                                                    : ReturnCodes::GenericError;
        resultData[1] = srvId;
        pushWord(startAddress, &resultData[2]);
        pushWord(endAddress, &resultData[4]);
        resultLength = 6;
      ...
      case SetGroupAddress: {
        ...
        popWord(startAddress, &data[2]);
        popWord(endAddress, &data[4]);
        resultData[0] = statusClearSetGroupAddress(startAddress, endAddress, true) ? ReturnCodes::Success
                                                                                   : ReturnCodes::GenericError;
        resultData[1] = srvId;
        pushWord(startAddress, &resultData[2]);
        pushWord(endAddress, &resultData[4]);
...
bool RouterObject::isRfSbcRoutingEnabled() { return _rfSbcRoutingEnabled; }

// TODO: check if IP SBC works the same way, just copied from RF
void RouterObject::functionIpEnableSbc(bool isCommand, uint8_t *data, uint8_t length, uint8_t *resultData,
                                       uint8_t &resultLength) {
  if (isCommand) {
...
// TODO: check if IP SBC works the same way, just copied from RF
bool RouterObject::isIpSbcRoutingEnabled() { return _ipSbcRoutingEnabled; }
